...
safety.nonlin2=alldata%>%
  ...
  geom_point()+geom_line(aes(group=treatment))+
  labs(y="Sum of adherence days",x="Week")
...
glm2=glm(factor(bin.AE)~trt*sequence2,data=dt2,family="binomial",
         alpha=training$bestTune[1],lambda=training$bestTune[2])
summary(glm2)
#ints not sig, remove

glm3=glm(factor(bin.AE)~trt+sequence2,data=dt2,family="binomial")
summary(glm3)
#sig diff in outcome btw seq 2 and 0




lasso.cv=glmnet::cv.glmnet(x=model.matrix(factor(bin.AE)~trt+sequence2, dt2),
                     y = dt2$bin.AE, family = "binomial", nfolds=10)
lasso=glmnet::glmnet(x=model.matrix(factor(bin.AE)~trt+sequence2, dt2),
                     y=dt2$bin.AE,lambda=lasso.cv$lambda.min,family="binomial")
...
summary(m5)
#one ints sig (trt B and seq 2 compared to trt A and seq 0), so sig carryover
#sig less total adherence days in B and C compared to A (shown in line plot)
...
#sig less adherence days in B and C vs. A
